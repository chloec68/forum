...
define('DS', DIRECTORY_SEPARATOR); // le caractère séparateur de dossier (/ ou \)
    // meilleure portabilité sur les différents systêmes : rendre le code indépendant du système d'exploitation.
    //define('DS'...) -> cette fonction peremt de définir une constante (ici DS) 
    //DIRECTORY_SEPARTOR -> constante prédéfinie PHP qui contient le séparateur de dossier du système d'exploitation 
define('BASE_DIR', dirname(__FILE__).DS); // pour se simplifier la vie
    //dirname(__FILE__) -> retourne le dossier parent du fichier (__FILE__) = constante magique PHP qui contient le chemin absolu (complet) du
    //fichier courant (index.php)
    //.DS -> concaténation du séparateur de dossier 
